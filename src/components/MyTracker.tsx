...
// Exported component: renders the logged-in user's entries
export function MyTrackerView({ myApps, allApps, onRefresh }: {
  myApps: Application[];
  allApps: Application[];
  onRefresh: () => void;
}) {
  return (
    <div className="page-enter">
      {myApps.map((app) => (
        <MyAppCard key={app.id} app={app} allApps={allApps} onRefresh={onRefresh} />
      ))}
    </div>
  );
}
