...
function MobileBottomNav({ pathname, unreadCount }: { pathname: string; unreadCount: number }) {
  ...
  return (
    <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-t border-sand-200 safe-area-bottom">
      <div ref={containerRef} className="flex items-center justify-around px-2 py-2.5 relative">
        {/* Sliding highlight */}
        <div
          className="absolute top-1.5 h-[calc(100%-12px)] bg-brand-500/10 rounded-xl transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] z-0"
          style={{ left: `${pill.left}px`, width: `${pill.width}px`, opacity: activeIdx >= 0 ? 1 : 0 }}
        />
        {BOTTOM_NAV.map((item) => {
          const active = pathname.startsWith(item.href);
          ...
          return (
            <Link
              ...
              className={cn(
                "relative z-10 flex flex-col items-center gap-1 px-3 py-1.5 min-w-[60px] transition-transform duration-150",
